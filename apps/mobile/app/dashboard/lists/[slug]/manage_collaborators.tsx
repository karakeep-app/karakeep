...
export default function ManageCollaboratorsScreen() {
  ...
  const readOnly = list?.userRole !== "owner";

  const invalidateListCaches = () =>
    Promise.all([
      utils.lists.getCollaborators.invalidate({ listId }),
      utils.lists.get.invalidate({ listId }),
      utils.lists.list.invalidate(),
      utils.bookmarks.getBookmarks.invalidate({ listId }),
    ]);

  // Fetch collaborators
  const { data: collaboratorsData, isLoading } =
    api.lists.getCollaborators.useQuery({ listId });
  ...
  if (listError) {
    return (
      <FullPageError error={listError.message} onRetry={() => refetchList()} />
    );
  }
  ...
  return (
    <CustomSafeAreaView>
      <Stack.Screen
        options={{
          headerTitle: readOnly ? "Collaborators" : "Manage Collaborators",
      ...
      <ScrollView className="flex-1 p-6">
        ...
        {/* Collaborators List */}
        <View className={!readOnly ? "mt-6" : ""}>
          <Text className="mb-3 text-sm font-semibold">
            {readOnly ? "People with Access" : "Current Collaborators"}
          </Text>
          {isLoading ? (
