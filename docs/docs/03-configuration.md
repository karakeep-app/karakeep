...
| ALLOW_INTERNAL_IP_REQUESTS | No       | false   | By default, Karakeep blocks worker-initiated requests whose DNS resolves to private, loopback, or link-local IP addresses. Set this to `true` only if you explicitly need to reach internal services and fully trust your network. |
