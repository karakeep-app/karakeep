...
| WORKERS_HOST                    | No                                    | 127.0.0.1       | Host to listen for requests to WORKERS_PORT. You will need to set this if running in a container, as localhost will not be reachable from outside                                                                                                                                                                 |
...
| WORKERS_DISABLED_WORKERS        | No                                    | Not set         | Comma-separated list of worker names to disable. This takes precedence over `WORKERS_ENABLED_WORKERS`.                                                                                                                                                                                                                       |
| DATA_DIR                        | Yes                                   | Not set         | The path for the persistent data directory. This is where the database resides. Assets are stored here by default unless `ASSETS_DIR` is set.                                                                                                                                                                                   |
| ASSETS_DIR                      | No                                    | Not set         | The path where crawled assets will be stored. If not set, it defaults to `${DATA_DIR}/assets`.                                                                                                                                                                                                                             |
| NEXTAUTH_URL                    | Yes                                   | Not set         | This should point to the address of your server. The app will function without it, but will redirect you to incorrect addresses, such as during sign-out.                                                                                                                                                                          |
...
| DISABLE_NEW_RELEASE_CHECK       | No                                    | false           | If set to true, the latest release check will be disabled in the admin panel.                                                                                                                                                                                                                                               |
| PROMETHEUS_AUTH_TOKEN           | No                                    | Random          | Enable a Prometheus metrics endpoint at `/api/metrics`. This endpoint requires the token to be passed in the Authorization header as a Bearer token. If not set, a new random token is generated every time at startup. This token cannot contain any special characters, or you may encounter a 400 Bad Request response. |
| RATE_LIMITING_ENABLED           | No                                    | false           | If set to true, API rate limiting will be enabled.                                                                                                                                                                                                                                                                      |
| DB_WAL_MODE                     | No                                    | false           | Enables WAL mode for the sqlite database. This should improve database performance. There's no reason not to set this to true, unless you're running the database on a network-attached drive. This will become the default in the future.                                                  |
