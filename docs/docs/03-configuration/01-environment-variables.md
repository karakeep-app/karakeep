...
| INFERENCE_JOB_TIMEOUT_SEC            | No       | 30                     | How long (in seconds) to wait for an inference job to finish before the worker gives up. The default of 30 s is sized for fast cloud APIs (e.g. OpenAI). For local Ollama on slow hardware, increase this together with `INFERENCE_FETCH_TIMEOUT_SEC` — the job is killed by whichever timeout fires first.                                                                          |
| INFERENCE_FETCH_TIMEOUT_SEC          | No       | 300                    | \[Ollama Only\] Timeout (in seconds) for the HTTP request to the Ollama server. On slow hardware, inference for large context windows can easily exceed the default 5 minutes. Set this (and `INFERENCE_JOB_TIMEOUT_SEC`) to a higher value, e.g. `900`, to allow longer runs.                                                                                                        |
