...
| CRAWLER_PARSE_MAX_OLD_SPACE_SIZE_MB      | No       | 512       | Maximum heap size in MB for the HTML parsing subprocess. If the parser OOMs on large pages, increase this value. The subprocess is isolated so an OOM won't crash the main worker.                                                                                                                                                                                             |
