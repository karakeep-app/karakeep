...
| CRAWLER_MONOLITH_ARGS                    | No       | []        | Include additional monolith arguments to be passed when archiving a full page, separated by `%%`. For example, `--no-fonts%%--no-audio%%--no-video` to exclude web fonts, audio, and video from archives. See [monolith options](https://github.com/nicholasgasior/monolith#options).                                                                                           |
