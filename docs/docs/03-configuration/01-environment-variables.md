...
| OAUTH_ID_TOKEN_SIGNED_RESPONSE_ALG          | No       | Not set                | The expected JWS signing algorithm for OIDC ID tokens. Set this when your provider signs ID tokens with an algorithm other than the default; Logto users should set `ES384`, and some providers use `ES256`. If this does not match the provider's ID token algorithm, OAuth login can fail during callback with an unexpected JWT algorithm error. Supported values: `RS256`, `RS384`, `RS512`, `PS256`, `PS384`, `PS512`, `ES256`, `ES384`, `ES512`, `EdDSA`. |
