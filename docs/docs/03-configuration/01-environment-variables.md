...
| CHAT_ENABLED                         | No       | false                  | Whether the experimental chat interface and chat backend endpoints are enabled. Set to `true` to show the chat UI and allow chat API requests.                                                                                                                                                                                                                                       |
| CHAT_MODEL                           | No       | INFERENCE_TEXT_MODEL   | The model to use for the experimental chat interface. If `OLLAMA_BASE_URL` is set, chat uses Ollama's OpenAI-compatible `/v1` API with this model. Otherwise chat uses `OPENAI_BASE_URL` when set, or OpenAI's API by default.                                                                                                                                              |
...
| EMBEDDING_ENABLE_AUTO_INDEXING       | No       | false                  | Whether bookmark embedding generation and vector indexing are enabled. When disabled, bookmark tagging still runs directly without waiting for embeddings.                                                                                                                                                                                                                              |
| EMBEDDING_TEXT_MODEL                 | No       | text-embedding-3-small | The model to be used for generating embeddings for the text.                                                                                                                                                                                                                                                                                                                          |
| EMBEDDING_CONTEXT_LENGTH             | No       | 8000                   | The max number of characters that will be passed to the embedding model. If the bookmark content is larger than this size, it will be truncated before embedding generation.                                                                                                                                                                                                            |
| EMBEDDING_DIMENSIONS                 | No       | 1536                   | The embedding vector dimensions expected by the vector store. This must match the embedding model or provider configuration.                                                                                                                                                                                                                                                           |
| EMBEDDING_NUM_WORKERS                | No       | 1                      | Number of concurrent workers for embedding generation and vector indexing tasks. Increase this if you have multiple embedding requests and want to process them in parallel.                                                                                                                                                                                                            |
| EMBEDDING_JOB_TIMEOUT_SEC            | No       | 60                     | How long to wait for an embedding generation and vector indexing job to finish before timing out. Increase this if your embedding provider or vector store is slow.                                                                                                                                                                                                                     |
...
| CRAWLER_PREFLIGHT_USER_AGENT             | No       | Not set   | The `User-Agent` header to send on the crawler's preflight request (the plain HTTP request that determines a link's content type and extracts early preview metadata before the full crawl). If not set, no `User-Agent` header is sent.                                                                                                                                        |
